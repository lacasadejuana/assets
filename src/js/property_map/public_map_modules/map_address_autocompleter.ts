...
export function createAutoCompleter({
    ...
    if (!globalThis.autocomplete || !singleInstance) {
        try {
            ...
            globalThis.autocomplete = autocomplete as google.maps.places.Autocomplete


            google.maps.event.addListener(globalThis.autocomplete, 'place_changed', function () {
                const place = globalThis.autocomplete.getPlace() as google.maps.places.PlaceResult,
                    { address_components, geometry } = place;
                ...
                if (!geometry || !geometry.location) {
                    ...
                    window.alert(
                        "No existe información georreferenciada para: '" + place.name + "'"
                    );
                    return;
                }
                console.log({ place });
