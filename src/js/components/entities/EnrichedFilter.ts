...
export class EnrichedFilter implements IEnrichedFiltro {
    filter_id: string;
    _campo_busqueda: string;
    _conector: string;
    _tipo_busqueda: VSearchType;

    _valor_busqueda: string | number | number[] | string[]
    disabled: boolean;
    index: number;
    readonly _filtro: IFiltro;
    constructor(filtro: IFiltro) {
        ...
        this.$store.active_filter = Alpine.store('active_filter') as ActiveFilterStore;
        this.$store.tipos_busqueda = Alpine.store('tipos_busqueda') as XTipoBusquedaStore;
    ...
    set tipo_busqueda(newType: VSearchType) {
        if (this._tipo_busqueda === newType) return;
        ...
        if (!isArray(this.valor_busqueda)) this.valor_busqueda = []
    ...
    get id_rol_negocio(): number | null {
        return (this.campo || { id_rol_negocio: null }).id_rol_negocio as number | undefined | null;
    }
    ...
    get campo() {
        return (this.filtro && this.campo_busqueda)
            ...
            : { id_input_type: VInputType.INPUT_TEXT } as unknown as NegocioColumn;
    }
    get tipo() {
    ...
    get properties() {
        return (this.campo && Array.isArray(this.campo.properties) ? this.campo.properties : []) as unknown as (TOption | TContactOption)[];
    }
    get items() {
        //properties.length > 0
        return (Array.isArray(this.valor_busqueda) ? this.valor_busqueda : [this.valor_busqueda]).map(String); //.split(',')
    }
    get options(): (TContactOption | TOption)[] {
        //@ts-ignore
        return this.campo ? this.campo.options : []
    }
    get operadores_busqueda(): Record<VSearchType, string> {
        //@ts-ignore

        return this.$store.tipos_busqueda.operadores_para(
            (this.campo || { id_input_type: 0 }) as unknown as NegocioColumn
        )
            || {
    ...
    get isValid() {
        return this.campo_busqueda && this.campo_busqueda !== ',negocio' && this.slug_name && (
            (this.valor_busqueda !== null && this.valor_busqueda !== undefined)
    ...
    get templateId() {
        if (this.renderButtonGroup) return '#render_button_group';
        if (this.renderSelect) return '#render_select';
        if (this.renderContacto) return '#render_contacto';
        if (this.renderDateRange) return '#render_date_range';
        if (this.renderSingleDate) return '#render_date';
        if (this.renderNull) return '#render_null';
        if (this.renderNumber) return '#render_number';
        if (this.renderText) return '#render_text';
        return '#render_text';
    }
    getTemplateContent() {
    ...
    get shouldRender(): TRenderValue {
        if (!this.campo) return 'renderNull'
        let {
            renderNumericInterval,
            renderSelect, renderSingleDate, renderDateRange, renderNumber, renderContacto, renderNull, renderText
            , renderButtonGroup,
        } = this;
        ...
        return (shouldRender ? shouldRender[0] : 'renderText') as TRenderValue
    ...
    get renderSelect() {
        ...
        return (this.campo.isSelectOrRadioButtonGroup
            ...
            //@ts-ignore
            || (!this.renderContacto && [search_types.IN, search_types.NOT_IN].includes(String(this.tipo_busqueda)) && (this.campo.options || []).length)
        ) && !this.renderNull && !this.renderButtonGroup
    }
