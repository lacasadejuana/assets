...
export type TloadPartialOptions = {
    display_filters_accordion: boolean,
    container_selector: string,
    pushState: boolean,
    clear_container: boolean
}
...
export function loadPartial(endpoint, options: TloadPartialOptions = {} as TloadPartialOptions) {
    let {
        display_filters_accordion = false,
        container_selector = '#barebone_container',
        pushState = true,
    ...
    let currentClassSlug = location.pathname.replace(/\//g, '_'),
        nextClassslug = endpointurl.pathname.replace(/\//g, '_'),
    ...
    let t_ini = performance.now(),
        t_ini_2 = Date.now()
    let container = $(container_selector),
        barebone_container = document.querySelector(container_selector) as HTMLElement;
    ...
    return Promise.all([
        ...
        if (clear_container) container.empty();
        ...
        //@ts-ignore
        if ($store && $store.user) $store.user.pathname = endpointurl.pathname
