...
//ColumnaVisible
import { ICamposBusqueda, IFilterStore, iCurrentColumns } from "@lacasadejuana/types";
...
export class ColumnasActualesStore extends CamposBusquedaStore implements iCurrentColumns {
    className = 'ColumnasActualesStore'
    default_columns: Array<NegocioColumn>;
    columnas_visibles: NegocioColumn[];
    protected properties: Array<NegocioColumn>;
    ...
    id = uuidv4()
    protected timerColor: string = 'color:emerald;font-weight:bold'
    protected classNameColor: string = 'color:darkcyan;font-weight:bold;'
    constructor() {
        super()
        this.ready = false
    ...
    async setDefaultColumns(default_columns: NegocioColumn[]) {
        this.default_columns = Array.from(new Set(default_columns));
        return this;

    }
    get active_filter() {
        return Alpine.store('active_filter') as IFilterStore
    }
    get columnVisibility() {
        return this.columnDefs.reduce((acc: Record<string, boolean>, col: NegocioColumn) => {
    ...
    reloadCampos(columnas_visibles: NegocioColumn[], reset: boolean = false): NegocioColumn[] {
        // don't accept empty arrays as input
        if (columnas_visibles.length === 0) return this.columnDefs
        /**
         * 
         */
        this.columnas_visibles = Array.from(new Set(columnas_visibles.filter(c => !defaultSlugs.includes(c.slug_name))))

        let visible_slugs = this.columnas_visibles.map(c => c.slug_name),
            default_slugs = this.default_columns.map(c => c.slug_name)
        ...
        this.properties = (this.default_columns || [])
            ...
            .map((c, index) => {
                c.is_default = true
                c.position = index
                if (['created_at', 'fechaCreacion', 'fecha_creacion_visual'].includes(c.slug_name)) c.position = 3 + index;
                return c;
            }).concat(
        ...
        this.properties = this.properties
            .map((col, index) => {
                //@ts-ignore
                let dttCol: NegocioColumn = this.campos_busqueda.find(col.slug_name) ?? new NegocioColumn(decorateCampo(col));
                ...
                if (col.visible !== undefined) {
                    if (['created_at', 'fechaCreacion', 'fecha_creacion_visual'].includes(dttCol.slug_name)) col.visible = false

                    dttCol.visible = col.visible
                }
                if (!this.active_filter || !this.active_filter.columnas_actuales) return dttCol;

                let colFiltro = this.active_filter.columnas_actuales.find(c => c.id == dttCol.id)
                if (colFiltro) {
                    dttCol.visible = colFiltro.visible
                }
                if (['created_at', 'fechaCreacion', 'fecha_creacion_visual'].includes(dttCol.slug_name)) dttCol.visible = false
                return dttCol
        ...
        if (!reset) this.properties.forEach(c => {
            ...
            if (['created_at', 'fechaCreacion', 'fecha_creacion_visual'].includes(c.slug_name)) c.visible = false
    ...
    syncWithActiveFilter() {
        Promise.all([
            ...
            this.properties = (this.properties || []).map(col => {
                if (!this.active_filter || !this.active_filter.columnas_actuales) return col;
                let colFiltro = this.active_filter.columnas_actuales.find(c => c.id == col.id)
                if (colFiltro) {
                    col.visible = colFiltro.visible
                }
                if (['created_at', 'fechaCreacion', 'fecha_creacion_visual'].includes(col.slug_name)) col.visible = false
    ...
    get campos_busqueda() {
        return Alpine.store('campos_busqueda') as ICamposBusqueda
    }
    get defaultSlugs() {
    ...
    refreshInvisibles(): ColumnasActualesStore {
    ...
    isDefaultField(slug_name: string) {
    ...
    * @returns 
    */
    find(slug_name): NegocioColumn {
        slug_name = slug_name.replace(/^([^,]+),?.*$/, '$1')
        //@ts-ignore
        return this.columnDefs.find(c => c.slug_name === slug_name) || {};
    }
    get(id): NegocioColumn {
        return this.columnDefs.find(c => c.id == id) as NegocioColumn;
    }
    ...
    toggle(slug_name) {
        ...
        let col = this.campos_busqueda.find(slug_name) as NegocioColumn
    ...
    get columnIds(): string[] {
        return this.columnDefs.map(c => c.id) as string[]
    }
    async moveColumn(oldIndex, newIndex) {
        ...
        console.info(
            ...
            `%c se ha movido %c${moved.slug_name} %c hasta después de %c ${(this.columnDefs[newIndex - 1] || { name: 'el final' }).slug_name}`,
            '',
    ...
    get featureProperties() {
        return this.columnDefs.filter(
            c => c.id_input_type !== VInputType.INPUT_CHECKBOX && c.id_input_type !== VInputType.INPUT_SELECT_MULTIPLE
        ).reduce((accum, campo) => {
            accum[
                campo.slug_name.replace('fecha_creacion_visual', 'created_at'
                )
...
export function columnas_actuales(): iCurrentColumns {
    return new ColumnasActualesStore() as unknown as iCurrentColumns
} 