...
import { staticFetchWrapper } from '../../../../../../negocios-panel/resources/js/components/decorators/staticFetchWrapper';
...
export const PublicLayerBarrios = ({ index, slug_name, name, layer_options }, comunas) => ({
    ...
    setStyle(): AlpineDataComponent<IGeoJsonLayerData> {
        ...
        requestAnimationPromise().then(() => {
            ...
            console.log('CALLING SET STYLE', { currentZoom })
            this.getLayer().setStyle((feature) => {

                let featureName = feature.getProperty(this.layer_options.labelProperty),
                    is_label = feature.getProperty('is_label'),
                    parent = layer.getFeatureById(String(feature.getId() || '').replace('-center', '')),
                    parentHighlighted = parent?.getProperty('highlighted');
                 ...
                 */
                let labelIsVisible = labelVisibility.always ||
                    (highlighted && (parentHighlighted || labelVisibility.highlighted)) ||
                    (labelVisibility.zoom && currentZoom >= labelVisibility.zoom),
                    labelClassName = highlighted ? "uppercase max-w-[125px] text-gray-500 markerLabel_break_words markerLabel bg-gray-200 radius-4   p-1 bg-opacity-30" : this.layer_options.labelClassName

                //  console.log({ comunaOffset, labelIsVisible, featureName, feature, labelVisibility, currentZoom })
    ...
    clickListener: null,
    callSetStyle() {
        console.info('calling set style')
        this.setStyle();
    },
    declareEventHandlers() {
        const labelProperty = this.layer_options.labelProperty || 'Nombre_de_Barrio'

        this.debouncedSetStyle = Alpine.debounce(this.callSetStyle.bind(this), 500)
