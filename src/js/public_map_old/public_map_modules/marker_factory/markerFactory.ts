...
const generateFatCanvas = function (options) {
	...
	let canvas = options.canvas || document.createElement("canvas"),
		anchorX = size.width / 2.5,
		anchorY = size.height - 1,
		radius = anchorX - 9,
		angulo = 1.1,
		fontFamily = options.fontFamily || "fontello",
...
function readCache(cacheKey, options) {
	...
	var cachedObj = JSON.parse(cached) as IIconOptions;
...
export function markerFactory(theoptions) {
	...
	if (iconObj && !theoptions.no_cache) return iconObj
	let size = { width: theoptions.canvasWidth || 96, height: theoptions.canvasHeight || 96 };
	let scale = theoptions.scale || 1,
		{ fillColor, strokeColor, fillOpacity, strokeOpacity } = theoptions



	if (
		theoptions.label.length === 4 ||
