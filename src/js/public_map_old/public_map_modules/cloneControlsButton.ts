...
export async function cloneControlsButton(
    gmap: google.maps.Map,
    text: string,
    attempt = 0,
    cloneFromSelector = '.gmnoprint .gm-style-mtc',
    controlPosition: google.maps.ControlPosition = google.maps.ControlPosition.TOP_LEFT,
    ...
    return Promise.resolve().then(() => {
        ...
        if (!lastContainer) {
            if (attempt < 5)
                return waitFor(1000).then(() => cloneControlsButton(
                    gmap,
                    text,
                    attempt++,
                    cloneFromSelector,
        ...
        let gmnoPrint = gmap.controls[controlPosition].getAt(0) as HTMLElement;
        if (!gmnoPrint || !gmnoPrint.classList.contains('gmnoprint')) {
            ...
            gmnoPrint.style.cssText = (lastContainer.parentElement as HTMLElement)?.style?.cssText;
            gmap.controls[controlPosition].push(gmnoPrint);
            gmnoPrint.style.marginLeft = `${-1 * Number(gmnoPrint.style.marginLeft.replace('px', ''))}px`
        }
        let UIButton = lastContainer.cloneNode(true) as HTMLElement;
