...
import { requestAnimationPromise, waitFor } from '../../../../../../negocios-panel/resources/js/components/plugins';
...
import { NegocioColumn } from '../../../../../../negocios-panel/resources/js/components/entities/NegocioColumn';

import { IKmzLayer } from '@lacasadejuana/types';
import { CamposBusquedaStore, ColumnasActualesStore } from '../../../../../../negocios-panel/resources/js/components';
...
export const PublicMapFrameData = () => {
    return {
        gmap: null,
        mapTypes: [],
        lastType: null,
        ...
        init() {
            ...
            this.$store.campos_busqueda.on('ready').then((columnas_actuales) => {
                this.campos = populateMapFields(
                    ...
                    this.$store.campos_busqueda as CamposBusquedaStore
                )
            });
            this.createMap().then(async (gmap) => {
                ...
                globalThis.layers = {};
                this.$store.public_maps.onLayersAdded().then(async () => {
                    this.appendFeatures()
                    this.fetchPage();
        ...
        async fetchPage() {
            ...
            if (this.$store.negocios.next_page_url && this.$store.negocios.data.length < this.$store.negocios.total) {
                ...
                let data = await this.$store.negocios.next() as Negocio[]
