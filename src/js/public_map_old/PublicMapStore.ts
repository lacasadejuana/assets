...
import { AlpineDataComponent, waitFor } from '../../../../../../negocios-panel/resources/js/components';
import { BaseClass } from '../../../../../../negocios-panel/resources/js/components/stores';
...
export class PublicMapStore extends BaseClass implements IMapStore {
    ...
    fetchPublicaciones() {
        let state = {
            limit: 1000,
            from: Number(Date.now() / 1000).toFixed(0)
        }
        return this.$store.negocios.fetchFilteredRecords('https://microserviceslocal.juana.house/api/maps/publicaciones', {
            method: 'post',
            headers: {},
            state,
            page: 1
    ...
    onLayersAddedListeners: THandler<this>[] = []
    onLayersAdded(handler: THandler<this>) {
        if (handler) {
