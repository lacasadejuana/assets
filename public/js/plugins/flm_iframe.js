var lfm_route = location.origin + '/filemanager';
...
$(document).on('click', '[data-display]', function () {
  let current_show_list = show_list,
    current_navbar_show_list = 'show_' + current_show_list;
  show_list = $(this).data('display');
  const content_alpine = document.querySelector('#content_alpine'),
    navbar_nav = document.querySelector('#nav-buttons .navbar-nav');
...
function toggleSelected(e) {
  ...
  console.log('toggleSelected', e.target, sequence);
...
function toggleActions() {
  ...
  $('[data-action=preview]').toggleClass('d-none', !(many_selected || one_selected));
...
function goTo(new_dir) {
  ...
  url.pathname = `/lfm${new_dir}`;
  history.pushState({ working_dir: new_dir }, null, url.toString());
  Alpine.store('files').properties = [];
...
function performLfmRequest(url, parameter, type) {
  ...
  return $.ajax({
    ...
    dataType: type || 'text',
    url: url.startsWith('/') ? url : (lfm_route + '/' + url),
    data: data,
    cache: false
  ...
  performLfmRequest('/api/lfm', {
    ...
    .done(function (data) {
      ...
      working_dir = response.working_dir,
        ...
        hasPaginator = !!response.paginator;

      Alpine.store('files').isEditable = response.isEditable;
      Alpine.store('files').nombre_campo = response.nombre_campo;
      Alpine.store('files').working_dir = response.working_dir;
      Alpine.store('files').id_negocio = response.id_negocio;
      Alpine.store('files').slug_name = response.slug_name;
      Alpine.store('files').nombre_campo = response.nombre_campo;
      Alpine.store('files').hashid_negocio = response.hashid_negocio;
      Alpine.store('files').hashid_campo = response.hashid_campo;
      Alpine.store('files').isEditable = response.isEditable;
      ...
      globalThis.inferPreviewInstance.files = [];
      ...
      if (hasItems) {
        ...
        items.forEach(function (item, index) {
          ...
          } else if (/^\s*(far|fas|fa) /.test(item.icon)) {

            var image = $('<div>').addClass(item.icon + ' d-flex justify-content-center');
          } else {
            var icon = $('<div>').addClass('ico');
            var image = $('<div>').addClass('mime-icon ico-' + item.icon).append(icon);
          }
          if (/^(http|https):\/\//.test(item.url)) {
            image.attr('rel', item.url);
          } else {
            newURL.pathname = item.url;
            image.attr('rel', newURL.toString());
          }
          item.rel = image.attr('rel');
          item.date = (new Date(item.time * 1000)).toLocaleString('en-GB');
          Alpine.store('files').properties.push(item);

          template.find('.square').append(image);
          template.find('.item_name').text(item.name);
          template.find('time').text(item.date);
