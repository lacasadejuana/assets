...
class BootstrapTable extends BootstrapTableOriginal {
    ...
    initAlternativeContainer() {
        ...
        const loadingTemplate = $.fn.bootstrapTable.utils.calculateObjectValue(this.options, this.options.loadingTemplate, [this.options.formatLoadingMessage()]);
        this.$container = $(`
        ...
        this.options.columns.forEach((columns, i) => {
          ...
          <div class="fht-cell"></div>
            ...
            columns.forEach((column, j) => {
                ...
                html.push(`<th${Utils.sprintf(' title="%s"', column.titleTooltip)}`, column.checkbox || column.radio ?
                    Utils.sprintf(' class="bs-checkbox %s"', column['class'] || '') :
                    classes || class_, Utils.sprintf(' style="%s"', halign + style + csses.join('; ')), Utils.sprintf(' rowspan="%s"', column.rowspan), Utils.sprintf(' colspan="%s"', column.colspan), Utils.sprintf(' data-field="%s"', column.field), 
                // If `column` is not the first element of `this.options.columns[0]`, then className 'data-not-first-th' should be added.
                j === 0 && i > 0 ? ' data-not-first-th' : '', '>');
                html.push(Utils.sprintf('<div class="th-inner %s">', this.options.sortable && column.sortable ? `sortable${columnHalign === 'center' ? ' sortable-center' : ''} both` : ''));
    ...
    initToolbar() {
        ...
        this.buttons = Object.assign(this.buttons, {
            ...
            ${this.constants.html.toolbarDropdown[0]}`);
                    if (opts.showColumnsSearch) {
                        html.push(Utils.sprintf(this.constants.html.toolbarDropdownItem, Utils.sprintf('<input type="text" class="%s" name="columnsSearch" placeholder="%s" autocomplete="off">', this.constants.classes.input, opts.formatSearch())));
                    ...
                    if (opts.showColumnsToggleAll) {
                        ...
                        html.push(Utils.sprintf(this.constants.html.toolbarDropdownItem, Utils.sprintf('<input type="checkbox" class="toggle-all" %s> <span>%s</span>', allFieldsVisible ? 'checked="checked"' : '', opts.formatColumnsToggleAll())));
                        html.push(this.constants.html.toolbarDropdownSeparator);
                    }
                    ...
                    this.columns.forEach((column, i) => {
                        ...
                        if (column.switchable) {
                            html.push(Utils.sprintf(this.constants.html.toolbarDropdownItem, Utils.sprintf('<input type="checkbox" data-field="%s" value="%s"%s%s> <span>%s</span>', column.field, i, checked, disabled, column.title)));
                            switchableCount++;
                        }
                    });
        ...
        for (const [buttonName, buttonConfig] of Object.entries(this.buttons)) {
            ...
            if ((!buttonConfig.hasOwnProperty('render') ||
        ...
        search-input" type="search" placeholder="${opts.formatSearch()}" autocomplete="off">`;
            ...
            if (opts.showSearchButton || opts.showSearchClearButton) {
                ...
                searchInputFinalHtml = opts.search ? Utils.sprintf(this.constants.html.inputGroup, searchInputHtml, buttonsHtml) : buttonsHtml;
            }
            ...
            this.data = searchText ? this.data.filter((item, i) => {
                for (let j = 0; j < this.header.fields.length; j++) {
                    ...
                    if (column && column.searchFormatter) {
                        value = Utils.calculateObjectValue(column, this.header.formatters[j], [value, item, i, column.field], value);
                    }
                    if (typeof value === 'string' || typeof value === 'number') {
                        if (this.options.strictSearch && (`${value}`).toLowerCase() === searchText ||
    ...
    initPagination() {
      ...
      ${paginationInfo}
        ...
        if (this.paginationParts.includes('pageList')) {
            html.push(`<div class="${this.constants.classes.pull}-${opts.paginationHAlign} pagination">`, Utils.sprintf(this.constants.html.pagination[0], Utils.sprintf(' pagination-%s', opts.iconSize)), Utils.sprintf(this.constants.html.paginationItem, ' page-pre', opts.formatSRPaginationPreText(), opts.paginationPreText));
            ...
            const pageItem = (i, classes = '') => Utils.sprintf(this.constants.html.paginationItem, classes + (i === opts.pageNumber ? ` ${this.constants.classes.paginationActive}` : ''), opts.formatSRPaginationPageText(i), i);
            if (from > 1) {
                ...
                if ((from - 1) === max + 1) {
                    ...
                    if ((from - opts.paginationPagesBySide * 2) > opts.paginationPagesBySide &&
    ...
    initRow(item, i, data, trFragments) {
        ...
        html.push('<tr', Utils.sprintf(' %s', htmlAttributes.length ? htmlAttributes.join(' ') : undefined), Utils.sprintf(' id="%s"', Array.isArray(item) ? undefined : item._id), Utils.sprintf(' class="%s"', style.classes || (Array.isArray(item) ? undefined : item._class)), Utils.sprintf(' style="%s"', Array.isArray(item) ? undefined : item._style), ` data-index="${i}"`, Utils.sprintf(' data-uniqueid="%s"', Utils.getItemField(item, this.options.uniqueId, false)), Utils.sprintf(' data-has-detail-view="%s"', (this.options.detailView && Utils.calculateObjectValue(null, this.options.detailFilter, [i, item])) ? 'true' : undefined), Utils.sprintf('%s', data_), '>');
        ...
        this.header.fields.forEach((field, j) => {
            ...
            cellStyle = Utils.calculateObjectValue(this.header, this.header.cellStyles[j], [value_, item, i, field], cellStyle);
            ...
            value = Utils.calculateObjectValue(column, this.header.formatters[j], [value_, item, i, field], value_);
            if (!(column.checkbox || column.radio)) {
    ...
    initBody(fixedScroll, updatedUid) {
        ...
        // show no records
        if (!hasTr) {
            this.$body.html(`<tr class="no-records-found">${Utils.sprintf('<td colspan="%s">%s</td>', this.getVisibleFields().length + Utils.getDetailViewIndexOffset(this.options), this.options.formatNoMatches())}</tr>`);
        }
    ...
    initServer(silent, query, url) {
        ...
        const request = $.extend({}, Utils.calculateObjectValue(null, this.options.ajaxOptions), {
            ...
            success: (_res, textStatus, jqXHR) => {
                const res = Utils.calculateObjectValue(this.options, this.options.responseHandler, [_res, jqXHR], _res);
                this.load(res);
    ...
    fitHeader() {
        ...
        $tr.find('> *').each((i, el) => {
            const $this = $(el);
            if (Utils.hasDetailViewIcon(this.options)) {
                if (i === 0 && this.options.detailViewAlign !== 'right' ||
    ...
    fitFooter() {
        ...
        $tr.find('> *').each((i, el) => {
            const $this = $(el);
            if (Utils.hasDetailViewIcon(this.options)) {
                if (i === 0 && this.options.detailViewAlign === 'left' ||
                    i === trLength - 1 && this.options.detailViewAlign === 'right') {
                    const $thDetail = $ths.filter('.detail');
                    const zoomWidth = $thDetail.innerWidth() - $thDetail.find('.fht-cell').width();
                    $thDetail.find('.fht-cell').width($this.innerWidth() - zoomWidth);
    ...
    getData(params) {
        ...
        if ((this.searchText ||
            ...
            !Utils.isEmptyObject(this.filterColumnsPartial)) && (!params || !params.unfiltered)) {
            data = this.data;
        }
...
$.fn.bootstrapTable = function (option, ...args) {
    ...
    this.each((i, el) => {
        ...
        const options = $.extend({}, BootstrapTable.DEFAULTS, $(el).data(), typeof option === 'object' && option);
