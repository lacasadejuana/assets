...
import { createAutoCompleter } from '../map_modules/map_address_autocompleter';
import { fillInAddress } from '../map_modules/map_geocoder';
const createTheFormStore = () => {
    return {
        ...
        pushFile(name, value) {
            let fileList = this.get(name) || { 0: {} }, maxEntry = Math.max(0, ...Object.keys(fileList).map(Number)), nextEntry = maxEntry + 1;
            fileList[nextEntry] = {
                id: nextEntry,
        ...
        getFileById(name, id) {
            let current = this.get(name) || { 0: {} };
            //@ts-ignore
            return Object.values(current).find(file => file.id == id);
        },
        ...
        updateFile(name, id, properties) {
            let fileList = this.get(name) || { 0: {} }, file = fileList[id];
            if (file) {
                file = {
                    ...file,
                    ...properties
        ...
        async updateFromLatLng(location) {
        ...
        populateFields() {
            ...
            //@ts-ignore
            "propiedad-direccion": direccion } = this.properties;
        ...
        populatePropertyForm(formData, include_empty = false) {
...
//# sourceMappingURL=createTheFormStore.js.map