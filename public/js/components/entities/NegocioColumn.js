...
import { processContactOption, processOption } from '../decorators';
export class NegocioColumn extends DttColumn {
    constructor(options) {
        super(options);
    ...
    get options() {
        ...
        return this.id_input_type == VInputType.INPUT_CONTACTO_ASOCIADO
            ? Object.values(this.properties).map(processContactOption)
    ...
    getShownValue(negocio) {
        ...
        if (this.isSelectOrRadioButtonGroup
            || this.isContact) {
            if (!this.optionMap) {
                console.warn('Campo select sin opciones', this);
                return baseline;
            }
            return this.optionMap.get(negocio[this.slug_name] || '')
                || this.optionMap.get(String(negocio[this.slug_name]) || '')
                || this.optionMap.get(Number(negocio[this.slug_name]) || '') || '';
        }
...
function assignInputType(dttCol) {
    /**
...
//# sourceMappingURL=NegocioColumn.js.map